...
const loggedOutNavItems = [
  { label: 'Home', href: '/' },
  { label: 'How It Works', href: '/#detection-flow' },
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[100] border-b border-white/10 bg-[#05090c]/75 text-[#f4fbff] shadow-[0_18px_45px_rgba(0,0,0,.22)] backdrop-blur-2xl">
      ...
      <div className="mx-auto flex h-[82px] w-[min(1240px,calc(100%-48px))] items-center justify-between gap-6 max-md:h-[72px] max-md:w-[min(680px,calc(100%-30px))]">
        <a className="group flex min-w-0 items-center gap-3.5" href="/" aria-label="Deep Fake Detector home">
          ...
          <span className="min-w-0">
            <span className="block text-[18px] font-black uppercase leading-none tracking-[.08em] text-white max-md:text-[15px]">Deep Fake <span className="text-cyan-300">Detector</span></span>
        ...
        <nav className="hidden items-center gap-1 rounded-full border border-white/10 bg-white/[.03] p-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,.06)] lg:flex" aria-label="Primary navigation">
          {navItems.map((item) => (
            <a className="rounded-full px-4 py-2.5 text-[12px] font-bold uppercase tracking-[.06em] text-slate-300 transition-all duration-300 hover:bg-cyan-300/10 hover:text-cyan-100 hover:shadow-[inset_0_0_0_1px_rgba(103,232,249,.18)]" href={item.href} key={item.label}>
              {item.label}
            </a>
          ))}
        </nav>
        ...
        <div className="hidden items-center gap-3 lg:flex">
          {user ? (
            ...
            <>
              <a className="inline-flex min-h-11 items-center justify-center rounded-full border border-white/10 bg-white/[.035] px-5 text-sm font-bold text-slate-200 transition-all duration-300 hover:border-cyan-300/30 hover:bg-cyan-300/10 hover:text-cyan-100" href="/signin">
                Sign In
              </a>
              <a className="inline-flex min-h-11 items-center justify-center rounded-full bg-cyan-300 px-5 text-sm font-extrabold text-[#021014] shadow-[0_14px_34px_rgba(34,211,238,.18)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-cyan-200 hover:shadow-[0_18px_42px_rgba(34,211,238,.28)]" href="/register">
                Get Started
              </a>
            </>
          )}
        ...
        <nav className="rounded-[28px] border border-white/10 bg-[#071116]/95 p-2 shadow-[0_24px_55px_rgba(0,0,0,.34)]" aria-label="Mobile navigation">
          ...
          {user ? (
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              <a className="flex min-h-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[.035] px-4 text-sm font-bold text-slate-100" href="/signin" onClick={() => setIsOpen(false)}>
                Sign In
              </a>
              <a className="flex min-h-12 items-center justify-center rounded-2xl bg-cyan-300 px-4 text-sm font-extrabold text-[#021014]" href="/register" onClick={() => setIsOpen(false)}>
                Get Started
              </a>
            </div>
          )}
